...
async function testWebhook() {
  const url = 'http://localhost:3000/api/email/inbound';
  ...
  const addFile = (name: string, filename: string, contentType: string, content: Buffer) => {
    body = Buffer.concat([
      body,
      Buffer.from(`--${boundary}\r\n`),
  ...
  addField('text', 'Please find the attached invoice. The PO is PO# 98765.');
