    ...
    return __generator(this, function (_c) {
      switch (_c.label) {
        ...
        case 1:
          ...
          return [
            4 /*yield*/,
            pdfDoc.embedFont(pdf_lib_1.StandardFonts.Helvetica),
          ];
        case 2:
          reg = _c.sent();
          return [
            4 /*yield*/,
            pdfDoc.embedFont(pdf_lib_1.StandardFonts.HelveticaBold),
          ];
        case 3:
          bold = _c.sent();
...
function drawIssuerBlock(page, d, invoice, reg, bold) {
  ...
  var finalIssuerLines =
    issuerLines.length > 0
      ? issuerLines
      : ["60 Romano Vineyard Way, #101", "North Kingstown, RI 02852"];
  for (
    var _i = 0, finalIssuerLines_1 = finalIssuerLines;
    _i < finalIssuerLines_1.length;
    _i++
  ) {
  ...
  d.text(
    firstNonEmpty(invoice.issuerEmail, "billing@system4ips.com"),
    LEFT,
    y,
    FS.body,
    reg,
...
function drawBillToBlock(d, invoice, reg, bold) {
  ...
  var lines = cleanLines(
    __spreadArray(
      [invoice.billToName],
      (_a = invoice.billToAddressLines) !== null && _a !== void 0 ? _a : [],
      true,
    ),
  );
...
function buildRows(invoice) {
  ...
  for (var _i = 0, lineItems_1 = lineItems; _i < lineItems_1.length; _i++) {
    ...
    if (item.type === "service") {
      ...
      if (!lines.length && !hasMoney(item.amount)) continue;
...
function buildServiceLines(item) {
  ...
  return raw.filter(function (line) {
    return !/see extracted text for details/i.test(line);
  });
}
...
function drawFooter(d, invoice, reg, bold, dividerY) {
      ...
      _c = cleanLines(
        __spreadArray(
          ...
          cleanLines(invoice.serviceAddressLines).length
            ...
            : (_a = invoice.billToAddressLines) !== null && _a !== void 0
              ? _a
              : [],
          true,
        ),
  ...
  for (
    var _d = 0,
      _e = cleanLines(
        (
          (_b = invoice.remitToLines) === null || _b === void 0
            ? void 0
            : _b.length
        )
  ...
  if (hasMoney(invoice.creditAmount) && invoice.creditAmount > 0) {
    ...
    d.rightText(
      formatCredit(invoice.creditAmount),
      summaryValueR,
      ys,
      FS.balance,
      bold,
  ...
  d.rightText(
    moneyText(invoice.balanceDue, invoice.totalAmount, invoice.subtotal),
    summaryValueR,
    ys - 2,
    14,
    bold,
...
function cleanLines(lines) {
  return lines
    .flatMap(function (line) {
      return String(line !== null && line !== void 0 ? line : "").split(
        /\r?\n/,
      );
    })
    .map(function (line) {
...
function moneyText() {
  ...
  for (var _a = 0, values_2 = values; _a < values_2.length; _a++) {
    ...
    if (hasMoney(value)) return (0, currency_1.formatCurrency)(value);
  }
  return "";
}
