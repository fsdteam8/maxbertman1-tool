...
export function buildOverlayOps(
  ...
  if (
    ...
    // Helper to check if a Y coordinate is within the Service Activity block
    // Remember: In PDF coordinates, topY > bottomY (Y increases downward in visual space)
    function isWithinServiceActivityBounds(y: number): boolean {
      // bottomY < y < topY in the coordinate system
      return y > bounds.bottomY && y < bounds.topY;
    }
