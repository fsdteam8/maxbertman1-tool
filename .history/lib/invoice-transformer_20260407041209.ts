...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // ─── INJECTION RULE ──────────────────────────────────────────────
    // Ensure PO# / W.O.# are consistently at the START of service line descriptions.
    // This handles all three cases:
    // 1. UPDATE existing W.O. → strips old and prepends new value
    // 2. ADD W.O. when it doesn't exist → prepends new value
    // 3. ADD W.O. after PO# → includes both in consistent format
    //
    ...
    try {
      ...
      if (isServiceLine && !looksLikeServiceAddress && (poNumber || woNumber)) {
        // Strip ALL existing PO# and W.O.# (at any position in the string)
        // This prevents duplication when updating values
        let cleaned = desc;
        ...
        // Build new prefix from provided values (this ensures UPDATE + ADD logic)
