  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    if (item.type === "service" && (poNumber || woNumber)) {
      ...
      if (!isServiceAddress) {
        ...
        // AND ALSO replace any existing old values - do both!
        ...
        if (woPlaceholder.detected && woNumber) {
          ...
          // Even if no placeholder, still replace existing W.O. number
          // This does surgical replacement: W.O. #03277024 -> W.O. #2580
          desc = desc.replace(
        ...
        if (poPlaceholder.detected && poNumber) {
          ...
          // Even if no placeholder, still replace existing PO/Order number
          if (/PO\s*#/i.test(desc)) {
            desc = desc.replace(
              /PO\s*#\s*[A-Za-z0-9\-\/\.]+/gi,
              `PO# ${poNumber}`
