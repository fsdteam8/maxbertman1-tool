  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    if (item.type === "service" && (poNumber || woNumber)) {
      ...
      if (!isServiceAddress) {
        ...
        // PRIORITY 2: Check for text-based placeholders AND ALSO replace existing old values
        const woPlaceholder = detectWOPlaceholder(desc);
        const poPlaceholder = detectPOPlaceholder(desc);

        // ALWAYS try to replace W.O. - whether there's a placeholder or existing value
        if (woNumber) {
          if (woPlaceholder.detected) {
            // Replace placeholder text like "pending"
            desc = replaceWOPlaceholder(desc, woNumber);
          }
          // ALSO always replace existing W.O. numbers (e.g., W.O. #03277024 -> W.O. #2580)
          desc = desc.replace(
