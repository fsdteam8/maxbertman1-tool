...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // For service items with PO/WO values, do PRECISE surgical replacement
    // Only replace the specific values, keep the rest of the layout intact
    if (item.type === "service" && (poNumber || woNumber)) {
      ...
      if (!isServiceAddress) {
        // STRATEGY: Replace ONLY the specific W.O./PO values
        // This preserves multi-line layouts with Order #, Ref #, dates, etc.
        
        // Replace W.O. value: "W.O. #03277023" -> "W.O. #2580"
        if (woNumber) {
          // Match "W.O. #XXX" or "W.O.#XXX" or variations
          desc = desc.replace(
        ...
        // Replace PO value: "PO# 123456" or "Order # 123456" -> "PO# 0321" or "Order # 0321"
        if (poNumber) {
          // Try PO# pattern first
          if (/PO\s*#/i.test(desc)) {
            desc = desc.replace(
              /PO\s*#\s*[A-Za-z0-9\-\/\.]+/gi,
              `PO# ${poNumber}`
