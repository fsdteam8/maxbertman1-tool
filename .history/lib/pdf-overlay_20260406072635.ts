...
export function buildOverlayOps(
  ...
  if (
    ...
    // SIMPLIFIED APPROACH: Repaint entire Service line with PO/W.O. at the beginning
    // This provides a cleaner layout and avoids overlay positioning issues
    if ((targetPo || targetWo) && serviceLines.length > 0) {
      // Use the first service line for PO/W.O. placement
      const targetLine = serviceLines[0];

      // Verify line is within Service Activity bounds
      if (targetLine.y > bounds.bottomY && targetLine.y < bounds.topY) {
        const firstItem = targetLine.items[0];
        const fontSize = firstItem.rect.height || 8;

        const minX = Math.min(...targetLine.items.map((i) => i.rect.x));
        const minY = Math.min(...targetLine.items.map((i) => i.rect.y)) - 1;
        const maxY =
          Math.max(...targetLine.items.map((i) => i.rect.y + i.rect.height)) + 1;

        // 1. Erase the entire original line (leaving room for Amount column on right)
        ...
        // 2. Build new text with PO/W.O. at the beginning, followed by original service text
        let newText = "";

        // Add PO# at the start if provided
        if (targetPo) {
          newText += `PO# ${targetPo}`;
        }

        // Add W.O.# after PO# (if provided)
        if (targetWo) {
          if (newText.length > 0) {
            newText += "  ";
          }
          newText += `W.O.# ${targetWo}`;
        }

        // Add original service description after PO/W.O.
        if (newText.length > 0) {
          newText += "  ";
        }
        newText += targetLine.text.trim();

        // 3. Draw repainted line with new text
        ops.push({
          pageIndex: firstItem.pageIndex,
          x: minX,
          y: targetLine.y - 2,
        ...
        console.warn(
          `[buildOverlayOps] First service line is outside Service Activity bounds (Y: ${bounds.bottomY}-${bounds.topY}, line Y: ${targetLine.y}). Skipping PO/W.O. rendering.`,
        );
      }
    }
  }
