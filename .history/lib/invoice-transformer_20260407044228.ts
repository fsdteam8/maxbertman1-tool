...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // ─── INJECTION RULE ──────────────────────────────────────────────
    // FULL LINE REPAINT: Only when user provides values AND no existing values detected.
    //
    // Two scenarios:
    // 1. PLACEHOLDER EXISTS or EXISTING VALUE EXISTS + User provides new value
    //    → Targeted replacement already done above, SKIP INJECTION RULE
    //    → Result: "W.O. # 123" (only the value changed)
    //
    // 2. NO PLACEHOLDER + NO EXISTING VALUE + User provides values
    //    → Run INJECTION RULE for full repaint with proper formatting
    //    → Result: "PO# 111  W.O# 222  Service description"
    //
    // This prevents unnecessary repainting when values already exist,
    // and ensures full repaint only when needed.
    //
    ...
    try {
      ...
      if (isServiceLine && !looksLikeServiceAddress && (poNumber || woNumber)) {
        ...
        // Also check if there are existing PO/W.O values in the ORIGINAL invoice
        // If they exist, they've already been replaced by replaceExistingPO/replaceExistingWO above
        const hasExistingPOorWO = !!invoice.poNumber || !!invoice.woNumber;

        // Only apply full INJECTION RULE if:
        // - NO placeholders were detected AND
        // - NO existing PO/WO values were in the original invoice
        // If either exists, targeted replacements above are sufficient
        if (!hasPlaceholder && !hasExistingPOorWO) {
        ...
        // If placeholder exists or existing values exist, targeted replacements above are sufficient
