...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    if (item.type === "service" && (poNumber || woNumber)) {
      const isServiceAddress =
        /Service\s+Address|SERVICE ADDRESS|[A-Z][a-z]+,?\s+[A-Z]{2}\s+\d{5}/i.test(desc);
      
      if (!isServiceAddress) {
        // PRIORITY 1: Check for token-based placeholders first
        if (hasWOToken(desc)) {
          const result = replaceWOToken(desc, woNumber);
          desc = result.text;
        } else if (hasWOPlaceholder(desc)) {
          // PRIORITY 2: Check for text-based placeholders (pending, awaiting, etc.)
          desc = replaceWOPlaceholder(desc, woNumber);
        }
        ...
        if (hasPOToken(desc)) {
          const result = replacePOToken(desc, poNumber);
          desc = result.text;
        } else if (hasPOPlaceholder(desc)) {
          // PRIORITY 3: Check for PO text-based placeholders
          desc = replacePOPlaceholder(desc, poNumber);
        }

        // PRIORITY 4: Generic replacement - but ONLY if no placeholders were found
        // This means: if there's NO placeholder, then do surgical value replacement
        const hasWOPlaceholderInDesc = hasWOToken(desc) || /pending|awaiting|tbd|tba/i.test(desc);
        const hasPOPlaceholderInDesc = hasPOToken(desc) || /pending|awaiting|tbd|tba/i.test(desc);

        if (!hasWOPlaceholderInDesc && woNumber) {
          // Surgical replacement: only replace the W.O. value, keep format
          desc = desc.replace(
        ...
        if (!hasPOPlaceholderInDesc && poNumber) {
          // Surgical replacement: only replace PO or Order # value, keep format
          if (/PO\s*#/i.test(desc)) {
            desc = desc.replace(
              /PO\s*#\s*[A-Za-z0-9\-\/\.]+/gi,
              `PO# ${poNumber}`
