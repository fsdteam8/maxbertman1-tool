 ...
 * 
 * STRATEGY 1: Placeholder-Based (Targeted Replacement)
 * - If W.O. placeholder exists: Replace only the placeholder keyword/value
 * - Minimal change, preserves original line structure
 * 
 * STRATEGY 2: New Value (Full Repaint)
 * - If NO W.O. placeholder but user provides value: Full repaint of service line
 * - Ensures clean format "PO# XXX  W.O# YYY  Description"
 * - Prevents UI overflow/mismatch issues
 */
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // ─── PRIORITY 1: Token-based replacement (e.g., {PO_WO#}) ──────
    if (/\{PO_WO#\}/.test(desc)) {
      const result = replacePOWOTokens(desc, poNumber, woNumber);
      desc = result.text;
    } else {
      // ─── PRIORITY 2: Individual token replacements ────────────────
      if (poNumber && hasPOToken(desc)) {
        const poResult = replacePOToken(desc, poNumber);
        desc = poResult.text;
      }

      // W.O. token replacement (only for service lines)
      ...
      // ─── PRIORITY 3: Placeholder-based replacement (Targeted) ─────
      // STRATEGY 1: If a W.O. PLACEHOLDER exists, use targeted replacement
      if (item.type === "service" && woNumber) {
        const woPlaceholderDetected = detectWOPlaceholder(desc).detected;
        
        if (woPlaceholderDetected) {
          // TARGETED REPLACEMENT: Replace only the placeholder keyword
          desc = replaceWOPlaceholder(desc, woNumber);
          // Skip the full repaint if placeholder was found and replaced
        } else {
          // STRATEGY 2: No placeholder exists - check for regex fallback
        ...
        // Non-service lines: standard PO replacement
        if (!hasPOToken(desc) && poNumber) {
          desc = replacePOPlaceholder(desc, poNumber);
          if (invoice.poNumber) {
            desc = replaceExistingPO(desc, invoice.poNumber, poNumber);
    ...
    // Same for title (apply matching logic)
    if (/\{PO_WO#\}/.test(title)) {
      ...
      if (item.type === "service") {
        ...
        // Handle W.O. placeholder in title
        if (woNumber) {
          const woPlaceholderDetected = detectWOPlaceholder(title).detected;
          if (woPlaceholderDetected) {
            title = replaceWOPlaceholder(title, woNumber);
          }
        }
    ...
    // ─── FULL REPAINT RULE (Strategy 2) ──────────────────────────────
    // If NO W.O. placeholder was detected AND user provides values:
    // REPAINT the entire service description with clean format
    // This prevents overflow/UI mismatch when adding new values
    try {
      ...
      // Check if W.O. placeholder exists (for targeted replacement detection)
      const hasWOPlaceholder = detectWOPlaceholder(item.description).detected;
      const hasPOPlaceholder = detectPOPlaceholder(item.description).detected;

      // FULL REPAINT: Only when NO placeholder exists and user provides NEW values
      // This handles the case: "Service description" → "PO# 111  W.O# 222  Service description"
      if (
        isServiceLine &&
        !looksLikeServiceAddress &&
        (poNumber || woNumber) &&
        !hasWOPlaceholder && // No W.O. placeholder was in original
        !hasPOPlaceholder    // No PO placeholder was in original
      ) {
        // Strip ALL existing PO#/W.O.# that might have been added by previous replacements
        let cleaned = desc;
        
        // Remove all PO#/W.O.# patterns to get clean service description
