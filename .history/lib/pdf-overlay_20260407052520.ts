...
export function buildOverlayOps(
  ...
  if (modifiedDescriptions.size > 0 && original.sourceMetadata.serviceActivityItems?.length) {
    ...
    let minServiceY = Infinity;
    let maxServiceY = -Infinity;
    
    for (let i = 0; i < original.lineItems.length; i++) {
      const origItem = original.lineItems[i];
      const newItem = markedUp.lineItems[i];
      
      if (origItem.type === "service" && origItem.description !== newItem.description) {
        // Find the first PDF item in serviceActivityItems that matches this service line
        // We use the first item we find and repaint from there
        if (!firstServiceItem) {
          // Get the first PDF item in service activity (skip header rows)
          for (const pdfItem of items) {
            if (!(/Service\s*-?\s*Activity|Services?|QTY|RATE|AMOUNT/i.test(pdfItem.fullText || ""))) {
        ...
        if (firstServiceItem) {
          // Find the vertical extent of ALL service description items
          // This includes W.O., Order #, Ref #, dates - all lines under the Service Activity column
          for (const item of items) {
            if (!(/Service\s*-?\s*Activity|Services?|QTY|RATE|AMOUNT/i.test(item.fullText || ""))) {
              minServiceY = Math.min(minServiceY, item.rect.y);
              maxServiceY = Math.max(maxServiceY, item.rect.y + item.rect.height);
            }
          }

          const fontSize = Math.max(firstServiceItem.rect.height * 0.9, 7);
          
          // ERASE THE ENTIRE SERVICE ACTIVITY COLUMN AREA
          // This covers all text from W.O. down through dates - removes everything
          ops.push({
            pageIndex: firstServiceItem.pageIndex,
            x: firstServiceItem.rect.x - 2,
            y: minServiceY - 2,
            width: (bounds.maxX || 300) - (firstServiceItem.rect.x - 2),
            height: maxServiceY - minServiceY + 4,
            newText: "",
            fontSize,
            isErase: true,
            align: "left",
          ...
          // Paint the new description starting at the top of the cleared area
          ops.push({
            pageIndex: firstServiceItem.pageIndex,
