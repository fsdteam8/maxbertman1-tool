...
/**
 * Apply both PO and WO replacements in a single pass for cleaner, atomic updates.
 * Prioritizes token-based replacement for items with {PO_WO#} combined token.
 */
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // PRIORITY 1: Combined token-based replacement (e.g., {PO_WO#})
    if (/\{PO_WO#\}/.test(desc)) {
      const result = replacePOWOTokens(desc, poNumber, woNumber);
      desc = result.text;
    } else {
      // PRIORITY 2: Apply individual token replacements
      if (poNumber && hasPOToken(desc)) {
        const poResult = replacePOToken(desc, poNumber);
        desc = poResult.text;
      }

      // W.O. replacements should only affect service activity lines
      if (item.type === "service") {
        if (woNumber && hasWOToken(desc)) {
          const woResult = replaceWOToken(desc, woNumber);
          desc = woResult.text;
        }
      }

      // PRIORITY 3: Fall back to regex-based replacement for legacy invoices
      if (!hasPOToken(desc) && poNumber) {
        desc = replacePOPlaceholder(desc, poNumber);
        // Always try to replace any existing PO value (not just invoice.poNumber)
        desc = replaceAnyExistingPO(desc, poNumber);
      }

      if (item.type === "service" && !hasWOToken(desc) && woNumber) {
        desc = replaceWOPlaceholder(desc, woNumber);
        // Always try to replace any existing W.O. value (not just invoice.woNumber)
        desc = replaceAnyExistingWO(desc, woNumber);
      }
    }

    // Same for title
    if (/\{PO_WO#\}/.test(title)) {
      const result = replacePOWOTokens(title, poNumber, woNumber);
      title = result.text;
    } else {
      if (poNumber && hasPOToken(title)) {
        const poResult = replacePOToken(title, poNumber);
        title = poResult.text;
      }
      // Only update W.O. in titles for service lines
      if (item.type === "service") {
        if (woNumber && hasWOToken(title)) {
          const woResult = replaceWOToken(title, woNumber);
          title = woResult.text;
        }
      }
      if (!hasPOToken(title) && poNumber) {
        title = replacePOPlaceholder(title, poNumber);
        // Always try to replace any existing PO value
        title = replaceAnyExistingPO(title, poNumber);
      }
      if (item.type === "service" && !hasWOToken(title) && woNumber) {
        title = replaceWOPlaceholder(title, woNumber);
        // Always try to replace any existing W.O. value
        title = replaceAnyExistingWO(title, woNumber);
      }
    }
    ...
    // ─── INJECTION RULE ──────────────────────────────────────────────
    // FULL LINE REPAINT: Only when user provides values AND no placeholders detected.
    //
    // Two scenarios:
    // 1. PLACEHOLDER EXISTS + User provides new value
    //    → Targeted replacement already done above, SKIP INJECTION RULE
    //    → Result: "W.O. # 123" (only the value changed)
    //
    // 2. NO PLACEHOLDER + User provides values
    //    → Run INJECTION RULE for full repaint with proper formatting
    //    → Result: "PO# 111  W.O# 222  Service description"
    //
    // This prevents unnecessary repainting of descriptions that already have
    // properly formatted placeholders, and ensures full repaint when needed.
    //
    // CRITICAL: Only applies to service-type items, NOT service address lines
    try {
      const isServiceLine = item.type === "service";
      const looksLikeServiceAddress =
        /Service\s+Address|SERVICE ADDRESS|[A-Z][a-z]+,?\s+[A-Z]{2}\s+\d{5}/i.test(
          desc,
        );

      if (isServiceLine && !looksLikeServiceAddress && (poNumber || woNumber)) {
        // Check if this item has placeholders that were just replaced
        const hasPlaceholder =
          detectPOPlaceholder(item.description).detected ||
          detectWOPlaceholder(item.description).detected ||
          hasPOToken(item.description) ||
          hasWOToken(item.description);

        // Only apply full INJECTION RULE if NO placeholders were detected
        // If placeholders exist, targeted replacements above already handled them
        if (!hasPlaceholder) {
          // FULL REPAINT: Strip ALL existing PO# and W.O.# and rebuild with new values
          // This ensures consistent formatting: "PO# 111  W.O# 222  Description"
          let cleaned = desc;
          
          // Remove all leading and embedded PO#/W.O.# patterns
          // More flexible regex: handles "W.O. #", "W.O.#", "WO #", "PO #", "PO#", etc.
          cleaned = cleaned.replace(
            /(?:^|\s+)?(?:PO\s*#?\s*[A-Za-z0-9\-\/\.]+|W\.?O\.?\s*#?\s*[A-Za-z0-9\-\/\.]+)(?:[\s,:-]{1,}|\s{2,})?/gi,
            " "
          );
          ...
          // Build new prefix from provided values
          const prefixParts: string[] = [];
          if (poNumber) prefixParts.push(`PO# ${poNumber}`);
          if (woNumber) prefixParts.push(`W.O# ${woNumber}`);

          if (prefixParts.length > 0) {
            // Prepend both PO# and W.O# to the cleaned description
            desc = `${prefixParts.join("  ")}${cleaned ? "  " + cleaned : ""}`.trim();
          } else {
            // If both cleared, just use cleaned description
            desc = cleaned.trim();
          }
        }
        // If placeholder exists, targeted replacements above are sufficient
        // Do NOT apply INJECTION RULE to avoid unnecessary full repaint
      }
    } catch (e) {
      // Non-fatal: preserve current desc on any unexpected error
      console.warn("[applyPOWOReplacement] Injection rule failed:", e);
    }
  ...
  return {
    ...
    woNumber: woNumber ?? invoice.woNumber,
  };
}
...
export function buildProcessedInvoice(
  ...
  const hasWoToken = original.lineItems.some((item) =>
    hasWOToken(item.description),
  );
  ...
  // Apply replacements using combined function (prioritizes token-based)
  if (
    (hasPoPlaceholder || hasWoPlaceholder || hasExistingPO || hasExistingWO) &&
    (finalPo || woNumber)
  ) {
    markedUp = applyPOWOReplacement(markedUp, finalPo, woNumber);
  }
