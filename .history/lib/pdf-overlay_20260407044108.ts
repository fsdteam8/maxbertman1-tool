...
export function buildOverlayOps(
  ...
  // ─── Render Updated Line Item Descriptions (from markedUp) ───
  // CRITICAL: This ensures descriptions updated by the transformer are rendered in the PDF.
  // This includes W.O./PO values that were updated or added during transformation.
  // We render full descriptions for service items that have changed.
  if (original.sourceMetadata.serviceActivityItems?.length) {
    const items = original.sourceMetadata.serviceActivityItems;
    const bounds = original.sourceMetadata.serviceActivityBounds;

    if (bounds) {
      // Process each service line item and render updated descriptions
      for (let i = 0; i < original.lineItems.length; i++) {
        const origItem = original.lineItems[i];
        const markedItem = markedUp.lineItems[i];

        // Only process service items with descriptions that have changed
        if (
          origItem.type === "service" &&
          origItem.description &&
          markedItem?.description &&
          origItem.description !== markedItem.description
        ) {
          // Find the corresponding PDF metadata item for this description
          const metaItem = items.find((it) =>
            origItem.description.includes((it.fullText || "").slice(0, 20)),
          );

          if (metaItem && metaItem.rect.y > bounds.bottomY && metaItem.rect.y < bounds.topY) {
            // Add operation to replace the entire description with the updated one
            // This erases the old text and writes the new formatted description
            ops.push({
              pageIndex: metaItem.pageIndex,
              x: metaItem.rect.x - 5,
              y: metaItem.rect.y - 2,
              width: metaItem.rect.width + 10,
              height: metaItem.rect.height * 2, // Allow for multi-line
              newText: markedItem.description,
              fontSize: Math.max(metaItem.rect.height * 0.9, 8),
              isErase: true,
              align: "left",
            });

            // Redraw the new description
            ops.push({
              pageIndex: metaItem.pageIndex,
              x: metaItem.rect.x,
              y: metaItem.rect.y,
              width: 0,
              height: Math.max(metaItem.rect.height * 0.9, 8),
              newText: markedItem.description,
              fontSize: Math.max(metaItem.rect.height * 0.9, 8),
              isErase: false,
              align: "left",
  ...
  // ─── Skip Complex PO/W.O. Logic If Descriptions Already Updated ───
  // The transformer has already updated descriptions with W.O./PO values.
  // We only need the complex dynamic logic if there are NO descriptions updated
  // (i.e., no placeholders were detected, so we still need to handle it here).
  
  const descriptionsUpdated = original.lineItems.some(
    (item, i) =>
      item.type === "service" &&
      item.description !== markedUp.lineItems[i]?.description,
  );

  // If no descriptions were updated, skip dynamic PO/W.O. rendering to avoid double-processing
  if (descriptionsUpdated) {
    console.log(
      "[buildOverlayOps] Service descriptions updated by transformer, skipping dynamic PO/W.O. logic",
    );
    return ops;
  }

  // ─── Dynamic Line-Replacement Logic (W.O. and Order/PO) ───
  // Only use this if the transformer did NOT update descriptions
  // (meaning there were no placeholders to detect/replace)
