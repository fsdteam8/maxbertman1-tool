...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // For service items with PO/WO values, use INJECTION RULE approach:
    // Strip all old references and rebuild cleanly
    if (item.type === "service" && (poNumber || woNumber)) {
      ...
      if (!isServiceAddress) {
        // STRATEGY: Complete description rebuild
        // 1. Strip ALL W.O./PO/Order/Ref patterns
        // 2. Keep only non-structural content
        // 3. Rebuild with new values at the start

        let cleaned = desc;
        
        // Remove W.O. patterns: W.O. #XXX, WO #XXX, W.O.#XXX, etc.
        cleaned = cleaned.replace(/W\.?O\.?[\s#]*[A-Za-z0-9\-\/\.]+/gi, "");
        
        // Remove Order # patterns
        cleaned = cleaned.replace(/Order\s*#\s*[A-Za-z0-9\-\/\.]+/gi, "");
        
        // Remove Ref # patterns
        cleaned = cleaned.replace(/Ref\s*#\s*[A-Za-z0-9\-\/\.]+/gi, "");
        
        // Remove PO patterns
        cleaned = cleaned.replace(/PO[\s#]*[A-Za-z0-9\-\/\.]+/gi, "");
        
        // Clean up dates, extra whitespace
        cleaned = cleaned.replace(/\d{1,2}\/\d{1,2}\/\d{4}/g, "");
        cleaned = cleaned.replace(/\s{2,}/g, " ").trim();
        
        // Build new description with values at start
