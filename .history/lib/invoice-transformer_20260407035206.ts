...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // PRIORITY 4: Append W.O# to descriptions with PO# but no W.O# placeholder
    // This handles the case where user inputs W.O# but there's no W.O# placeholder
    // Only append to activity lines (not Service Address, emails, etc)
    if (item.type === "service" && woNumber && !hasWOToken(item.description)) {
      const woAppendResult = appendWOToExistingPO(desc, woNumber);
      if (woAppendResult.wasAppended) {
        desc = woAppendResult.text;
      }
    }

    // INJECTION RULE: Ensure PO# / W.O.# tokens are PREPENDED at start of
    // activity descriptions when values are provided. Replace existing
    // leading tokens to avoid duplication; remove only cleared values.
    // Never touch Service Address-like lines.
    try {
      const isServiceLine = item.type === "service";
      const looksLikeServiceAddress = /Service\s+Address|SERVICE ADDRESS|[A-Z][a-z]+,?\s+[A-Z]{2}\s+\d{5}/i.test(
        desc,
      );

      if (isServiceLine && !looksLikeServiceAddress) {
        // Strip existing leading PO/W.O. fragments repeatedly
        let cleaned = desc;
        let prev = null;
        const leadingTokenRegex = /^\s*(?:PO#\s+[A-Za-z0-9\-\/\.]+|W\.O\.#\s+[A-Za-z0-9\-\/\.]+)(?:[\s,:-]{1,}|\s{2,})?/i;
        do {
          prev = cleaned;
          cleaned = cleaned.replace(leadingTokenRegex, "");
        } while (cleaned !== prev);

        // Build new prefix from provided values (current state)
        const prefixParts: string[] = [];
        if (poNumber) prefixParts.push(`PO# ${poNumber}`);
        if (woNumber) prefixParts.push(`W.O.# ${woNumber}`);

        if (prefixParts.length > 0) {
          desc = `${prefixParts.join("  ")}  ${cleaned}`.trim();
        } else {
          // If both cleared, ensure we removed previous tokens but keep rest
          desc = cleaned.trim();
        }
      }
    } catch (e) {
