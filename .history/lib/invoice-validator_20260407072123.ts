 ...
 * - All monetary calculations use precise Decimal arithmetic for 100% accuracy
 */
...
export function validateInvoiceData(
  ...
  if (
    ...
    const expectedTax = new Decimal(serviceTotal)
      .applyTaxPercent(invoice.taxRate)
      .round();
  ...
  // Use Decimal for precise balance calculation
  const serviceDec = new Decimal(serviceTotal);
  const taxDec = new Decimal(taxTotal);
  const creditDec = new Decimal(creditTotal);
  const expectedTotal = serviceDec.add(taxDec).subtract(creditDec).round();
