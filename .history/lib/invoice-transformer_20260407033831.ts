...
import { ensureLeadingPOWOTokens } from "@/lib/token-replacement";
...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    // Ensure leading PO/W.O# tokens are present (or cleaned) at the beginning
    // of activity descriptions according to the current values. Skip Service Address
    desc = ensureLeadingPOWOTokens(desc, poNumber ?? undefined, woNumber ?? undefined);
    title = ensureLeadingPOWOTokens(title, poNumber ?? undefined, woNumber ?? undefined);
