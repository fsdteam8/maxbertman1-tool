...
export function applyMarkupToInvoice(
  ...
  if (invoice.subtotal !== null && subtotalRaw > 0) {
    ...
    const discrepancy = Math.abs(subtotalRaw - expectedSubtotal) / expectedSubtotal;
    if (discrepancy > 0.05) {
      console.warn(
  ...
  if (subtotalRaw > 0) {
    ...
    } else if (invoice.taxAmount !== null && invoice.subtotal && invoice.subtotal > 0) {
  ...
  const balanceDueRaw =
    totalAmountRaw !== null
      ? totalAmountRaw - (creditAmountRaw ?? 0)
      : null;

  // ─── Step 6: Final Rounding (ONLY AT OUTPUT) ─────────────────────
  const subtotal = subtotalRaw > 0 ? round2(subtotalRaw) : null;
  const taxAmount = taxAmountRaw !== null && taxAmountRaw > 0 ? round2(taxAmountRaw) : null;
  const creditAmount =
