...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    if (woNumber && !hasWOToken(item.description)) {
    ...
    // PRIORITY 5: Ensure leading PO#/W.O# prefix on activity descriptions.
    // Rules:
    // - Only modify `service` line items (preserve tax/credit/other)
    // - If a PO/W.O. value is provided, ensure it appears at the START of the description
    // - If a value was cleared (undefined/null), remove only that leading token
    // - If a leading PO/W.O# already exists, replace its value rather than duplicating
    if (item.type === "service") {
      // Match existing leading tokens (examples: "PO# 12345", "W.O# ABC", "PO# 123  W.O# ABC")
      const leadingRegex = /^(?:PO#\s+([^\s]+))?(?:\s*\s*)(?:W\.O#\s+([^\s]+))?\s*/i;
      const m = desc.match(leadingRegex);
      let rest = desc;
      let existingPo = null as string | null;
      let existingWo = null as string | null;
      if (m) {
        if (m[1]) existingPo = m[1];
        if (m[2]) existingWo = m[2];
        // Remove the matched leading portion from rest
        rest = desc.replace(leadingRegex, "").trimStart();
      }

      // Determine desired leading tokens based on provided values
      const parts: string[] = [];
      if (poNumber) {
        parts.push(`PO# ${poNumber}`);
      }
      if (woNumber) {
        parts.push(`W.O# ${woNumber}`);
      }

      if (parts.length > 0) {
        // Prepend or replace leading tokens
        desc = parts.join("  ") + (rest ? `  ${rest}` : "");
      } else {
        // No PO/WO provided: if there were existing leading tokens, remove them
        if (existingPo || existingWo) {
          desc = rest;
        }
      }
    }
