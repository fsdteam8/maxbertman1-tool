...
/**
 * Update existing W.O# or append W.O# to descriptions.
 * Handles three cases:
 * 1. W.O# already exists: UPDATE it with new value (don't duplicate)
 * 2. PO# exists but no W.O#: APPEND W.O# after the PO#
 * 3. Neither exists but it's a service line: ADD W.O# as new field
 *
 * This prevents W.O# from being added to non-activity descriptions.
 * Only processes if:
 * 1. Description is not a "Service Address" line (city, state, zip)
 * 2. Only called for service-type line items
 */
export function appendWOToExistingPO(
  ...
  // This ensures W.O is ONLY added to Service Activity descriptions
  if (/[A-Z][a-z]+,?\s+[A-Z]{2}\s+\d{5}|Service\s+Address/i.test(text)) {
    return { text, wasAppended: false };
  }

  // CASE 1: Update existing W.O# value (replace old with new, don't duplicate)
  // Pattern: "W.O# <old-value>" or "WO <old-value>" anywhere in text
  const existingWOPattern = /W\.?O\.?\s*#?\s*([A-Za-z0-9\-\/\.]+)/i;
  if (existingWOPattern.test(text)) {
    const newText = text.replace(existingWOPattern, `W.O# ${woNumber}`);
  ...
  // CASE 2: Append W.O# after PO# if PO# exists but no W.O# yet
  ...
  // CASE 3: Add W.O# as new field (no PO#, but it's service activity)
  // Just append W.O# at the start or end of service description
  // This handles the case where user adds W.O# to an invoice with no PO#
  if (!text.includes("W.O#") && !text.includes("W.O ") && text.trim()) {
    // Prepend W.O# to make it prominent
    const newText = `W.O# ${woNumber}  ${text}`;
    return { text: newText, wasAppended: true };
  }
