 ...
 * CRITICAL: Uses precise decimal arithmetic for 100% accuracy.
 * Preserves dollar sign and commas if present in original string.
 */
function addMarkupOp(ops: OverlayOp[], item: TextItem): void {
  ...
  // Use Decimal for precise 1% markup calculation
  const updated = new Decimal(original).applyMarkup(1).round();
