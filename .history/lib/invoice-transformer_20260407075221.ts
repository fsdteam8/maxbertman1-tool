...
export function applyMarkupToInvoice(
  ...
  // NOTE: Skip TAX items here - they will be recalculated in Step 3
  const markedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    // Skip tax items in markup - they will be recalculated based on new subtotal
    if (item.type === "tax") {
      return item; // Keep original tax item for now, will be replaced later
    }

    // Apply markup to service and credit items using Decimal arithmetic
    return {
      ...item,
      amount:
        item.amount !== null
  ...
  if (invoice.subtotal !== null && subtotal > 0) {
    const expectedSubtotal = new Decimal(invoice.subtotal)
      ...
      .round();
    const discrepancy =
      Math.abs(subtotal - expectedSubtotal) / expectedSubtotal;
    if (discrepancy > 0.05) {
      console.warn(
  ...
  if (subtotal > 0) {
    ...
    if (invoice.taxRate !== null) {
      ...
      // Derive tax rate from original values, normalized to percentage
      // CRITICAL: Calculate as percentage, not as multiplier
      const impliedTaxRate = (invoice.taxAmount / invoice.subtotal) * 100;
      taxRate = Math.round(impliedTaxRate * 10000) / 10000; // 4 decimal precision
    } else if (invoice.taxAmount !== null && invoice.taxAmount > 0) {
      const effectiveSubtotal = invoice.subtotal || subtotal;
      const impliedTaxRate = (invoice.taxAmount / effectiveSubtotal) * 100;
      taxRate = Math.round(impliedTaxRate * 10000) / 10000;
    } else {
      // No tax in original, don't add tax
      totalAmountDec = subtotalDec;
    }

    // Apply tax calculation using Decimal arithmetic
    // Instead of: total = subtotal * (1 + rate/100)
    // Use: tax = subtotal * rate/100, then total = subtotal + tax
    // This avoids floating-point multiplier errors
    if (totalAmountDec === null && taxRate !== null) {
      const taxRateDecimal = Math.round(taxRate * 10000) / 10000; // Ensure 4 decimals
      taxAmountDec = new Decimal(subtotal).multiply(taxRateDecimal / 100);
      totalAmountDec = subtotalDec.add(taxAmountDec);
    }
  }
  ...
  const totalAmount = totalAmountDec !== null ? totalAmountDec.round() : null;
  const creditAmount =
    creditAmountDec !== null ? creditAmountDec.round() : null;
  ...
  // ─── Step 6: Update line items with CORRECT values ──────────────────────
  // CRITICAL: Replace tax line item amount with recalculated value
  // This ensures that when code sums lineItems, it gets the correct total
  const finalLineItems = markedLineItems.map((item) => {
    // Replace tax line item with recalculated amount
    if (item.type === "tax" && taxAmount !== null) {
      console.log(
        `[applyMarkupToInvoice] Updating tax line item: ${item.amount} → ${taxAmount}`,
      );
      return {
        ...item,
        amount: taxAmount,
  ...
  console.log(
    `[applyMarkupToInvoice] Final calculation: subtotal=$${subtotal?.toFixed(2)}, tax=$${taxAmount?.toFixed(2)}, total=$${totalAmount?.toFixed(2)}, balanceDue=$${balanceDue?.toFixed(2)}`,
  );
