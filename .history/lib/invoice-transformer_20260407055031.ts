...
export function applyPOWOReplacement(
  ...
  const updatedLineItems: ParsedLineItem[] = invoice.lineItems.map((item) => {
    ...
    if (item.type === "service" && (poNumber || woNumber)) {
      ...
      if (!isServiceAddress) {
        // PRIORITY 1: Check for token-based placeholders first (e.g., {WO#}, {PO#}, {PO_WO#})
        ...
        // PRIORITY 2: Check for text-based placeholders (pending, awaiting, tbd, etc.)
        // Only replace if a placeholder is detected - this is targeted replacement
        const woPlaceholder = detectWOPlaceholder(desc);
        const poPlaceholder = detectPOPlaceholder(desc);

        if (woPlaceholder.detected && woNumber) {
          desc = replaceWOPlaceholder(desc, woNumber);
        }
        if (poPlaceholder.detected && poNumber) {
          desc = replacePOPlaceholder(desc, poNumber);
        }

        // PRIORITY 3: Generic surgical replacement
        // Only if NO placeholders were detected - this preserves multi-line layouts
        if (!woPlaceholder.detected && woNumber && !hasWOToken(desc)) {
        ...
        if (!poPlaceholder.detected && poNumber && !hasPOToken(desc)) {
