...
async function main() {
  ...
  // Step 2: Apply business rules with a test PO
  ...
  console.log(`  - markedUp.woNumber: ${processed.markedUp.woNumber}`);
  ...
  poWoOps.forEach((op, i) => {
    ...
    console.log(
      `    - Dims: ${op.width.toFixed(1)}x${op.height.toFixed(1)}`,
    );
  ...
  if (bounds) {
    ...
    poWoOps.forEach((op, i) => {
      ...
      console.log(
        `  - Op ${i + 1} within bounds: Y=${withinY} X=${withinX}`,
      );
    });
  }
}
