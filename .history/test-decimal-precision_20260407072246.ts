...
const testValues = [90.01, 100.00, 99.99, 0.01, 1234.56, 50.005];
...
problematicValues.forEach(({ val, desc }) => {
  ...
  console.log(`${desc}`);
  console.log(`  Decimal: $${precise.toFixed(2)}, FP: $${floatingPoint.toFixed(2)}, Match: ${match}`);
});
...
const items = [10.01, 20.02, 30.03, 15.50, 24.44];
...
console.log(`Individual items marked up and summed (new): $${items.reduce((acc, val) => acc + new Decimal(val).applyMarkup(1).round(), 0).toFixed(2)}`);
