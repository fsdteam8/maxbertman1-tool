...
export default function ToolPage() {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    console.log("[Tool] File selected:", selected?.name, selected?.type, selected?.size);
    if (selected) {
      ...
      if (!isPDF) {
        ...
        console.error("[Tool] Invalid file type:", selected.type, "name:", selected.name);
        setError(msg);
        // Reset the input
        e.target.value = "";
        return;
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (dropped) {
      ...
      if (!isPDF) {
        console.error("[Tool] Invalid dropped file type:", dropped.type, "name:", dropped.name);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Failed to process the invoice.";
